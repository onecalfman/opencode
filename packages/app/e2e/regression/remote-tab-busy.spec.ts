...
test("tab busy indicator reflects the tab server's own status and keeps its project tag visible", async ({ page }) => {
  ...
  await expect(tabB.locator("[data-project-tag]")).toHaveText("🚀/β")
  ...
  await expect(tabA.locator("[data-project-tag]")).toHaveText("serv")
...
async function mockServers(page: Page) {
  await page.route("**/*", async (route) => {
    ...
    if (url.pathname === "/project" || url.pathname === "/project/current" || url.pathname === "/api/project") {
      const project = {
        id: current.projectID,
        worktree: current.directory,
        vcs: "git",
        tag: url.origin === serverB ? "🚀/β" : undefined,
        time: { created: 1, updated: 1 },
        sandboxes: [],
      }
      return json(route, url.pathname.endsWith("/project") ? [project] : project)
    }
    if (url.pathname === "/api/project/current") return json(route, { id: current.projectID, directory: current.directory })
